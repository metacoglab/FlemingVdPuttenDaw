    ...
    for k = 1:blockNo;
        if compute_conditions
            ...
            for c = 1:length(coherence)
                precoh_index(precoh(k,:) == coherence(c)) = c;
                postcoh_index(postcoh(k,:) == coherence(c)) = c;
            end
            precoh_index = precoh_index - 2;
            postcoh_index = postcoh_index - 2;
        ...
        durations{1} = 0.3;
        pmod(1).name{1} = 'Pre coherence';
        pmod(1).param{1} = precoh_index(~err(k,:));    % put on [-1 0 1] scale
        pmod(1).poly{1} = 1;
        pmod(1).name{2} = 'RT';
        pmod(1).param{2} = zscore(log(rt(k, ~err(k,:))));
        pmod(1).poly{2} = 1;
        orth{1} = 0;
        
        names{2} = 'Post, correct';
        onsets{2} = postOnsets(~err(k,:) & accuracy(k,:) == 1);
        durations{2} = 0.3;
        pmod(2).name{1} = 'Post coherence';
        pmod(2).param{1} = postcoh_index(~err(k,:) & accuracy(k,:) == 1);
        pmod(2).poly{1} = 1;        
        orth{2} = 0;
        
        names{3} = 'Post, error';
        onsets{3} = postOnsets(~err(k,:) & accuracy(k,:) ~= 1);
        durations{3} = 0.3;
        pmod(3).name{1} = 'Post coherence';
        pmod(3).param{1} = postcoh_index(~err(k,:) & accuracy(k,:) ~= 1);  
        pmod(3).poly{1} = 1;        
        orth{3} = 0;
        
        names{4} = 'Conf';
        onsets{4} = confOnsets(~err(k,:));
        durations{4} = 0;
        orth{4} = 0;
