...
vector<lower=0,upper=1>[N] model_conf;
  ...
  for (i in 1:N) {
    if (i <= ST) {   
        ...
        // different weighting for pre and post depending on confirmatory/disconfirmatory evidence
        if (((x2[i] > 0) && (a[i] == 1)) || ((x2[i] < 0) && (a[i] == 0)))
          loglikC = loglikC_pre + w1*loglikC_post  + brt*log(rt[i]);
        else
          loglikC = loglikC_pre + w2*loglikC_post  + brt*log(rt[i]);
